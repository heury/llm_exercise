...
def init_model_dllm(config, from_weight, device):
    tokenizer = AutoTokenizer.from_pretrained('../../minimind_model', local_files_only=True)
    model = MiniMindForMaskedDiffusion(config)
    if from_weight and from_weight != 'none':
        moe_suffix = '_moe' if config.use_moe else ''
        ckp = f'../../minimind_out/{from_weight}_{config.hidden_size}{moe_suffix}.pth'
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", type=str, default="../../minimind_dataset/sft_t2t_mini.jsonl", help="훈련 데이터 경로")
