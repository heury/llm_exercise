...
def convert_torch2transformers(torch_path, transformers_path, lm_config, dtype=torch.bfloat16):
    ...
    model = MiniMindOmni(lm_config, audio_encoder_path="../../models/SenseVoiceSmall", vision_model_path="../../models/siglip2-base-p32-256-ve")
    ...
    print(f"Transformers 형식으로 저장되었습니다: {transformers_path}")


def convert_transformers2torch(transformers_path, torch_path):
    model = AutoModelForCausalLM.from_pretrained(transformers_path, trust_remote_code=True)
    torch.save(model.state_dict(), torch_path)
    print(f"PyTorch 형식으로 저장되었습니다: {torch_path}")


if __name__ == '__main__':
    lm_config = OmniConfig(hidden_size=768, num_hidden_layers=8, use_moe=False)
    torch_path = f"../../checkouts/sft_omni_{lm_config.hidden_size}{'_moe' if lm_config.use_moe else ''}.pth"
