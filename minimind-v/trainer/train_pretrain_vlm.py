...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="MiniMind-V 사전학습")
    parser.add_argument("--save_dir", type=str, default="./checkouts", help="모델 저장 디렉터리")
    parser.add_argument('--save_weight', default='pretrain_vlm', type=str, help="저장할 가중치 파일의 접두사")
    parser.add_argument("--epochs", type=int, default=2, help="학습 에폭 수")
    parser.add_argument("--batch_size", type=int, default=16, help="배치 크기")
    parser.add_argument("--learning_rate", type=float, default=4e-4, help="초기 학습률")
    ...
    parser.add_argument('--max_seq_len', default=450, type=int, help="학습 시 최대 절단 길이")
    parser.add_argument('--use_moe', default=0, type=int, choices=[0, 1], help="MoE 아키텍처 사용 여부(0=아니오, 1=예)")
    parser.add_argument("--data_path", type=str, default="./datasets/sft_vlm_english_flickr8k_mini.parquet", help="학습 데이터 경로")
    parser.add_argument('--from_weight', default='full_sft', type=str, help="어떤 가중치에서 학습을 시작할지 지정합니다. none이면 기반 가중치 없이 학습합니다")
    parser.add_argument('--from_resume', default=0, type=int, choices=[0, 1], help="자동 감지 후 이어서 학습할지 여부(0=아니오, 1=예)")
    parser.add_argument('--freeze_llm', default=2, type=int, choices=[0, 1, 2], help="동결 정책(0=전체 학습, 1=동결 후 첫/마지막 레이어만 해제, 2=전체 동결 후 proj만 학습)")
    ...
    parser.add_argument("--wandb_project", type=str, default="MiniMind-V-Pretrain", help="wandb 프로젝트 이름")
