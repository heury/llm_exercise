...
# 方式2：基于 PyTorch 模型（确保./checkouts目录下有对应权重）
...
从下文提供的[数据集下载链接](https://www.modelscope.cn/datasets/gongjy/datas/files) 下载所需数据文件，并放入 `./datas` 目录
...
**断点续训说明：**
- 训练过程会自动在 `./checkouts/` 目录保存完整检查点（模型、优化器、训练进度等）
...
> 训练后，将得到 `checkouts/pretrain_*.pth` 作为输出权重（其中 `*` 为模型 dimension，默认为 `768`）
...
> 训练后，将得到 `checkouts/full_sft_*.pth` 作为输出权重（其中 `full` 表示全参数微调）

#### 2.3 测试已训练模型（可选）

确保待测试的模型 `*.pth` 文件位于 `./checkouts/` 目录下；也可直接前往[此处](https://www.modelscope.cn/models/gongjy/minimind-3-pytorch/files)下载我已训练好的 `*.pth` 权重。
...
MiniMind训练数据集下载地址： [ModelScope](https://www.modelscope.cn/datasets/gongjy/datas/files) | [HuggingFace](https://huggingface.co/datasets/jingyaogong/datas/tree/main)

> 无需全部clone，可单独下载所需的文件

将下载的数据集文件放到`./datas/`目录下（✨为推荐的必须项）
...
minimind-3-exam 不是更大的基座模型，也几乎没有额外注入新知识。它仅基于 minimind-3 在 [lora_exam.jsonl](https://huggingface.co/datasets/jingyaogong/datas/blob/main/lora_exam.jsonl) 上做了一次轻量 LoRA 对齐，再将 [lora_exam_768.pth](https://huggingface.co/jingyaogong/minimind-3-pytorch/resolve/main/lora_exam_768.pth) 合并回基模后的结果。这部分数据由 ceval 与 mmlu (英文) 的 test 子集抽样构成，并做了前缀、后缀等格式增强，主要作用是对齐选择题评测中常见的上下文与候选项组织形式，而不是学习题目答案。
