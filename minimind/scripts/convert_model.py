...
def convert_torch2transformers_minimind(torch_path, transformers_path, dtype=torch.float16):
    ...
    print(f"모델이 Transformers-MiniMind 형식으로 저장되었습니다: {transformers_path}")


# QwenForCausalLM/LlamaForCausalLM 구조와 호환되는 생태계
def convert_torch2transformers(torch_path, transformers_path, dtype=torch.float16):
    ...
    if int(transformers.__version__.split('.')[0]) >= 5:
        ...
        config['rope_theta'] = lm_config.rope_theta; config['rope_scaling'] = None; del config['rope_parameters']
        json.dump(config, open(config_path, 'w', encoding='utf-8'), indent=2, ensure_ascii=False)
    print(f"모델이 Transformers 형식으로 저장되었습니다: {transformers_path}")
...
def convert_merge_base_lora(base_torch_path, lora_path, merged_torch_path):
    ...
    print(f"LoRA가 병합되어 기본 모델 구조의 PyTorch 형식으로 저장되었습니다: {merged_torch_path}")
...
def convert_json_to_jinja(json_file_path, output_path):
    ...
    print(f"템플릿이 jinja 파일로 저장되었습니다: {output_path}")
...
if __name__ == '__main__':
    ...
    torch_path = f"../../checkouts/full_sft_{lm_config.hidden_size}{'_moe' if lm_config.use_moe else ''}.pth"
    transformers_path = '../minimind-3'
    convert_torch2transformers(torch_path, transformers_path)

    # # merge lora
    # base_torch_path = f"../../checkouts/full_sft_{lm_config.hidden_size}{'_moe' if lm_config.use_moe else ''}.pth"
    # lora_path = f"../../checkouts/lora_identity_{lm_config.hidden_size}{'_moe' if lm_config.use_moe else ''}.pth"
    # merged_torch_path = f"../../checkouts/merge_identity_{lm_config.hidden_size}{'_moe' if lm_config.use_moe else ''}.pth"
    # convert_merge_base_lora(base_torch_path, lora_path, merged_torch_path)

    # convert_transformers2torch(transformers_path, torch_path)
    # convert_json_to_jinja('../../models/tokenizer_config.json', '../../models/chat_template.jinja')
    # convert_jinja_to_json('../../models/chat_template.jinja')
