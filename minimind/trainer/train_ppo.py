...
# MiniMindLM을 상속하는 커스텀 Critic 모델
class CriticModel(MiniMindForCausalLM):
    def __init__(self, params):
        super().__init__(params)
        # lm_head를 단일 값을 출력하는 선형 레이어로 교체
        self.value_head = nn.Linear(params.hidden_size, 1)

    def forward(self, input_ids=None, attention_mask=None, **kwargs):
        # 기본 모델로 hidden states를 얻음
        outputs = self.model(input_ids=input_ids, attention_mask=attention_mask, **kwargs)
        hidden_states = self.model.norm(outputs[0])
        # value_head로 value 추정값을 얻음
...
def ppo_train_epoch(epoch, loader, iters, rollout_engine, ref_model, actor_scheduler, critic_scheduler, reward_model, start_step=0, wandb=None, use_sglang=False):
    ...
    for step, batch in enumerate(loader, start=start_step + 1):
        ...
        with torch.no_grad():  # 롤아웃 단계에서는 old_logp와 old_values를 얻기 위해 추론만 수행하고, 메모리 절약을 위해 그래디언트를 끊습니다
            ...
            token_rewards[torch.arange(B, device=args.device)[valid_resp], last_idx[valid_resp]] += rewards[valid_resp]  # 마지막에 외부 리워드 추가
        ...
        for ppo_epoch in range(args.ppo_update_iters):
            ...
            for i in range(0, B, mb_size):
                ...
                # 특정 rank만 중단되고 다른 rank가 계속되어 DDP 교착이 생기지 않도록 GPU 간 approx_kl 동기화
                ...
                # 조기 중단 시에도 forward-backward 루프를 유지해야 하므로 DDP 통신은 끊지 않고 loss만 잘라냅니다
                if stop_ppo:
        ...
        if (step % args.save_interval == 0 or step == iters) and is_main_process():
            ...
            # critic을 포함한 전체 상태를 저장하기 위해 lm_checkpoint 사용
            lm_checkpoint(lm_config, weight=args.save_weight, model=actor_model, optimizer=actor_optimizer, 
                         epoch=epoch, step=step, wandb=wandb, save_dir='../checkouts',
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="MiniMind PPO(근접 정책 최적화)")
    parser.add_argument("--save_dir", type=str, default="../checkouts", help="모델 저장 디렉터리")
    parser.add_argument('--save_weight', default='ppo_actor', type=str, help="저장할 가중치 파일의 접두사")
    parser.add_argument("--epochs", type=int, default=1, help="학습 에폭 수")
    parser.add_argument("--batch_size", type=int, default=2, help="배치 크기")
    parser.add_argument("--learning_rate", type=float, default=3e-7, help="Actor학습률")
    parser.add_argument("--critic_learning_rate", type=float, default=5e-7, help="Critic학습률")
    ...
    parser.add_argument("--data_path", type=str, default="../datasets/rlaif.jsonl", help="RLAIF 데이터 경로")
    ...
    parser.add_argument("--kl_coef", type=float, default=0.02, help="KL 발산 패널티 계수")
    ...
    parser.add_argument("--cliprange_value", type=float, default=0.2, help="가치 함수 클리핑 범위")
    parser.add_argument("--ppo_update_iters", type=int, default=2, help="동일 롤아웃 배치에 대한 반복 업데이트 횟수")
