...
def dpo_loss(ref_log_probs, policy_log_probs, mask, beta):
    # ref_log_probs와 policy_log_probs는 모두 (batch_size, seq_len) 형태입니다
    ref_log_probs = (ref_log_probs * mask).sum(dim=1)
    policy_log_probs = (policy_log_probs * mask).sum(dim=1)

    # chosen 데이터와 rejected 데이터 분리
...
if __name__ == "__main__":
    ...
    parser.add_argument("--learning_rate", type=float, default=4e-8, help="초기 학습률(망각 방지를 위해 <=5e-8 권장)")
    ...
    parser.add_argument("--data_path", type=str, default="../datasets/dpo.jsonl", help="DPO 학습 데이터 경로")
    ...
    Logger(f'정책 모델 전체 파라미터 수: {sum(p.numel() for p in model.parameters()) / 1e6:.3f} M')
    # 참조 모델 초기화(ref_model은 동결됨)
    ref_model, _ = init_model(lm_config, args.from_weight, device=args.device)
    ref_model.eval()
    ref_model.requires_grad_(False)
    Logger(f'참조 모델 전체 파라미터 수: {sum(p.numel() for p in ref_model.parameters()) / 1e6:.3f} M')
