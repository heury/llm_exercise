...
# ======== 도구 정의 ========
TOOLS = [
    {"type": "function", "function": {"name": "calculate_math", "description": "Evaluate a mathematical expression", "parameters": {"type": "object", "properties": {"expression": {"type": "string"}}, "required": ["expression"]}}},
    ...
    {"type": "function", "function": {"name": "get_exchange_rate", "description": "Look up exchange rates", "parameters": {"type": "object", "properties": {"from_currency": {"type": "string"}, "to_currency": {"type": "string"}}, "required": ["from_currency", "to_currency"]}}},
...
def rollout_single(rollout_engine, tokenizer, messages, tools, max_turns=3, max_new_tokens=256, thinking_ratio=0.5, device="cuda"):
    ...
    for turn in range(max_turns):
        ...
        for call in calls:
            ...
            result_str = (json.dumps(result, ensure_ascii=False) if result else '{"error": "tool not found"}')[:2048]  # 매우 큰 숫자로 토크나이저가 넘치지 않도록 방지
...
def calculate_rewards(prompts, completions, gt_batch, tools_batch, num_gen, reward_model=None, device="cuda", turn_outputs_batch=None, unfinished_batch=None):
    ...
    for idx, response in enumerate(completions):
        ...
        reward -= 0.5 * sum(abs(turn.count('<tool_call>') - turn.count('</tool_call>')) for turn in turn_answers)  # 태그 패널티
        # -------- 도구 호출 없음: 형식 및 리워드 모델 점수 --------
        if not tool_calls:
            ...
            rewards[idx] = max(min(reward, 3.0), -3.0)  # 전체 점수 클리핑
        # -------- 도구 호출 있음: 실행 결과 리워드 --------
        else:
            ...
            tool_gap = abs(valid_call_count - len(gt)) + max(0, len(tool_calls) - valid_call_count)  # 도구 개수 차이
            ...
            if unfinished: reward -= 0.5  # 미완료 패널티
...
if __name__ == "__main__":
    ...
    parser.add_argument('--from_weight', default='full_sft', type=str, help="불러올 사전학습 가중치 이름")
    parser.add_argument('--from_resume', default=0, type=int, choices=[0, 1], help="체크포인트에서 재개할지 여부")
    parser.add_argument("--use_wandb", action="store_true", help="wandb로 로깅할지 여부")
    ...
    Logger(f'리워드 모델 로드 위치 {args.reward_model_path}')
    # 롤아웃 엔진
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if skip > 0:
            Logger(f'Epoch [{epoch+1}/{args.epochs}]: 처음 {start_step} 스텝을 건너뜀')
