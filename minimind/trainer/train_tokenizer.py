# 참고: tokenizer(어휘)를 다시 학습하는 것은 권장하지 않습니다. MiniMind에는 이미 tokenizer가 포함되어 있으며, 이 스크립트는 학습과 참고용입니다. 서로 다른 어휘로 학습한 모델은 출력이 일관되지 않고 커뮤니티의 모델 재사용성을 낮춥니다.
# 참고: tokenizer를 다시 학습하는 것은 권장하지 않습니다. MiniMind에는 이미 tokenizer가 포함되어 있습니다. 이 스크립트는 학습과 참고용입니다. 서로 다른 tokenizer로 학습한 모델은 출력이 일관되지 않고 모델 재사용성을 낮춥니다.
import os
import json
...
def get_texts(data_path):
    with open(data_path, 'r', encoding='utf-8', errors='ignore') as f:
        for i, line in enumerate(f):
            if i >= 10000: break # 테스트용으로 10,000줄 선택
            try:
...
def train_tokenizer(data_path, tokenizer_dir, vocab_size, special_tokens_num=SPECIAL_TOKENS_NUM):
    ...
    buffer_tokens = [f"<|buffer{i}|>" for i in range(1, num_buffer + 1)] # 일정 수의 토큰 슬롯 예약
...
def eval_tokenizer(tokenizer_dir):
    ...
    messages = [
        {"role": "system", "content": "당신은 훌륭한 챗봇이며 항상 정확한 응답을 제공합니다!"},
        {"role": "user", "content": '어디에서 왔나요?'},
        {"role": "assistant", "content": '저는 달에서 왔습니다'},
        {"role": "user", "content": '정확히 어디에서 왔나요?'},
    ...
    print(new_prompt)
    print('-'*100)
    print('토크나이저 어휘 크기:', len(tokenizer))
    model_inputs = tokenizer(new_prompt)
    print('인코더 길이:', len(model_inputs['input_ids']))
    response = tokenizer.decode(model_inputs['input_ids'], skip_special_tokens=False)
    print('디코더 일관성:', response == new_prompt, "\n")
    print('-'*100)
    print('압축률 테스트(문자/토큰):')
    test_texts = [
        # 중국어 샘플(약 200자)
        "Artificial intelligence is a branch of computer science that seeks to understand the nature of intelligence and create machines that respond in ways similar to human intelligence. Research in this field includes robotics, speech recognition, image recognition, natural language processing, and expert systems. Since its birth, AI theory and technology have matured, and its applications have continued to expand. Future AI products may become containers for human knowledge. AI can simulate information processes related to human consciousness and thought. It is not human intelligence, but it can think in human-like ways and may even surpass human intelligence.",
        "Interstellar travel refers to voyages through space within or even between galaxies. Because space is vast, traditional chemical rockets are inadequate for journeys between stars. Scientists have proposed many options, including ion thrusters, nuclear thermal rockets, and even the idea of using antimatter as an energy source. Science-fiction concepts such as warp drives and wormhole travel are also repeatedly explored in theoretical physics. Although humans have so far only set foot on the Moon, breakthroughs in nuclear fusion and materials science may make travel to Mars and the farther edges of the solar system possible.",
        # 영어 샘플(약 200단어/문자)
