"""
MiniMind 지식 증류(Knowledge Distillation) 학습 스크립트

교사 모델(teacher)의 소프트 출력을 활용하여 학생 모델(student)을 학습시킨다.
CE 손실과 KL 발산 기반 증류 손실을 결합하여 최종 손실을 계산한다.
...
# 증류 손실 계산: 교사 모델의 소프트 확률과 학생 모델의 로그 확률 간 KL 발산을 구한다.
...
# 한 에포크 증류 학습을 수행한다. CE 손실과 증류 손실을 결합하여 학생 모델을 최적화한다.
def train_epoch(epoch, loader, iters, teacher_model, lm_config_student, start_step=0, wandb=None, alpha=0.0, temperature=1.0):
    ...
    # 교사 모델을 평가 모드로 설정하고 기울기 계산을 비활성화
    ...
    for step, (input_ids, labels) in enumerate(loader, start=start_step + 1):
        ...
        # 교사 모델 순전파 (eval 모드 & no_grad에서만 실행)
        if teacher_model is not None:
            with torch.no_grad():
        ...
        # 3) 총 손실 = alpha * CE + (1-alpha) * Distill
        ...
        if (step % args.save_interval == 0 or step == iters) and is_main_process():
            ...
            lm_checkpoint(lm_config_student, weight=args.save_weight, model=model, optimizer=optimizer, scaler=scaler, epoch=epoch, step=step, wandb=wandb, save_dir='../../minimind_out/checkpoints')
            model.train()
            del state_dict
...
if __name__ == "__main__":
    # MoE 모델로 dense 모델을 증류하거나, 더 큰 teacher_hidden_size 모델로 더 작은 student_hidden_size 모델을 증류할 수 있다
    ...
    parser.add_argument('--save_weight', default='full_dist', type=str, help="저장 가중치 접두사 이름")
    ...
    parser.add_argument("--accumulation_steps", type=int, default=1, help="기울기 누적 스텝 수")
    parser.add_argument("--grad_clip", type=float, default=1.0, help="기울기 클리핑 임계값")
    ...
    parser.add_argument("--max_seq_len", type=int, default=340, help="학습 최대 시퀀스 길이 (중국어 1토큰 약 1.5~1.7자)")
    parser.add_argument("--data_path", type=str, default="../../minimind_dataset/sft_t2t_mini.jsonl", help="학습 데이터 경로")
    ...
    parser.add_argument('--student_use_moe', default=0, type=int, choices=[0, 1], help="학생 모델 MoE 사용 여부 (0=아니오, 1=예)")
    parser.add_argument('--teacher_use_moe', default=1, type=int, choices=[0, 1], help="교사 모델 MoE 사용 여부 (0=아니오, 1=예)")
    parser.add_argument('--from_student_weight', default='full_sft', type=str, help="학생 모델 기반 가중치")
    parser.add_argument('--from_teacher_weight', default='full_sft', type=str, help="교사 모델 기반 가중치")
    parser.add_argument('--from_resume', default=0, type=int, choices=[0, 1], help="자동 감지 및 이어 학습 여부 (0=아니오, 1=예)")
    parser.add_argument('--alpha', default=0.5, type=float, help="CE 손실 가중치, 총 손실=alpha*CE+(1-alpha)*KL")
    ...
    ckp_data = lm_checkpoint(lm_config_student, weight=args.save_weight, save_dir='../../minimind_out/checkpoints') if args.from_resume==1 else None
    ...
    Logger(f'학생 모델 총 파라미터 수: {sum(p.numel() for p in model.parameters()) / 1e6:.3f} M')
    teacher_model, _ = init_model(lm_config_teacher, args.from_teacher_weight, device=args.device)
    teacher_model.eval()
    teacher_model.requires_grad_(False)
    Logger(f'교사 모델 총 파라미터 수: {sum(p.numel() for p in teacher_model.parameters()) / 1e6:.3f} M')
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if skip > 0: 
            Logger(f'Epoch [{epoch + 1}/{args.epochs}]: 앞의 {start_step}개 step 건너뛰고, step {start_step + 1}부터 시작')
